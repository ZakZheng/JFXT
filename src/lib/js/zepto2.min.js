...
$.actions=function(params){params=$.extend({},defaults,params);show(params);$('.weui_mask_visible').bind("touchmove",function(e){ e.preventDefault();});$('.weui_actionsheet').bind("touchmove",function(e){ e.preventDefault();});}
...
;(function($){$.fn.searchBar=function(options){options=$.extend({focusingClass:'weui_search_focusing',searchText:"搜索",cancelText:"取消"},options);var html='<div class="weui_search_bar">'+
                    ...
                    '<a href="javascript:" class="weui_search_cancel">'+options.cancelText+'</a></div>';var $search=$(html);this.prepend($search);const $searchBar=this.find('.weui_search_bar');const $searchText=this.find('.weui_search_text');const $searchInput=this.find('.weui_search_input');this.on('focus','#weui_search_input',function(){$searchText.hide();$searchBar.addClass(options.focusingClass);bindEvent($searchInput,'onfocus',options);}).on('blur','#weui_search_input',function(){$searchBar.removeClass(options.focusingClass);!!$(this).val()?$searchText.hide():$searchText.show();bindEvent($searchInput,'onblur',options);}).on('touchend','.weui_search_cancel',function(){$searchInput.val('');bindEvent($searchInput,'oncancel',options);}).on('touchend','.weui_icon_clear',function(e){e.preventDefault();$searchInput.val('');if(document.activeElement.id!='search_input'){$searchInput.trigger('focus');}
